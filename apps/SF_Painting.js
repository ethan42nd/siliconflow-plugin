...
async function getMemoryCollector() {
    if (!memoryCollector) {
        const module = await import('./MemoryCollector.js');
        memoryCollector = new module.UserMemory();
    }
...
async function getUserMemoryPrompt(groupId, userId, config) {
    try {
        ...
        const memoryText = await mc.getMemoryForPrompt(String(groupId), String(userId), {
            maxFacts: 8
        });
        ...
        return `\n\n【关于对方的信息】\n${memoryText}\n\n请自然地参考以上信息，但不要刻意提及你"记得"这些信息，要像朋友一样自然地交流。`;
    } catch (error) {
        logger.debug(`[记忆注入] 获取用户记忆失败: ${error}`);
        return '';
    }
}
...
export class SF_Painting extends plugin {
    ...
    async sf_chat(e, config_date = undefined) {
        ...
        // 提取@信息并在文本中提供QQ号（便于工具调用）
        let atInfo = '';
        if (e.message && Array.isArray(e.message)) {
            const atList = e.message.filter(m => m.type === 'at' && m.qq && m.qq !== Bot.uin);
            if (atList.length > 0) {
                try {
                    ...
                    for (const at of atList) {
                        const qq = at.qq;
                        // 注：保留头像图片，让AI可以查看/描述头像
                        // 同时在文本中提供QQ号，便于工具调用
                        if (memberMap) {
                            const info = memberMap.get(Number(qq));
                            if (info) {
                                atTexts.push(`${info.card || info.nickname}(QQ号: ${qq})`);
                                continue;
                            }
                        }
                        atTexts.push(`用户(QQ号: ${qq})`);
                    }
                    atInfo = `艾特了 ${atTexts.join('、')}`;
                } catch (err) {
                    logger.debug(`[SF插件]处理@信息失败: ${err}`);
                }
            }
        }
        ...
        // 注入用户记忆
        if (e.isGroup) {
            const memoryPrompt = await getUserMemoryPrompt(e.group_id, e.user_id, config_date);
            if (memoryPrompt) {
                systemPrompt += memoryPrompt;
            }
        }

        // 将@信息添加到用户消息中（在atInfo处理之后）
        if (atInfo) {
            toAiMessage = `${atInfo}，${toAiMessage}`;
        }

        // 如果启用了工具调用，在系统提示中添加工具使用说明
        if (config_date.smartMode?.tools?.enable) {
            systemPrompt += `

【工具调用说明】
你可以使用以下工具来帮助用户：
1. 禁言/解禁工具：当用户要求禁言某人或解除禁言时，使用 muteTool，需要提供目标用户的 QQ 号
2. 戳一戳工具：当用户要求戳某人时，使用 pokeTool，需要提供目标用户的 QQ 号
3. 点赞工具：当用户要求给某人点赞时，使用 likeTool，需要提供目标用户的 QQ 号
4. 成员信息查询：当用户要求查询某人信息时，使用 memberInfoTool

重要提示：
- 当用户@某人时，QQ号已经在消息中提供（格式：用户名(QQ号: xxx)），同时会附带对方的头像图片
- 工具参数中的 target/qq 只需要填写纯数字 QQ 号，不要带其他文字
- 如果需要禁言，时间单位是秒（1分钟=60秒，1小时=3600秒）
- 判断逻辑：如果用户要求"禁言/戳/赞"等操作 → 使用工具；如果用户要求"看头像/描述图片" → 直接描述图片内容`;
        }
