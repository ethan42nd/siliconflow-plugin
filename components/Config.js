...
class Config {
  ...
  enhanceConfig(config, configName) {
    if (configName === 'config') {
      // 读取gemini额外的模型列表
      const defaultGeminiModels = ['gemini-3.1-pro-preview', 'gemini-3-pro-preview', 'gemini-3-flash-preview', 'gemini-2.5-pro', 'gemini-2.5-flash', 'gemini-3-pro-image-preview', 'gemini-2.5-flash-image-preview']
      try {
